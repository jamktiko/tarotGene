    ...
    {#each pakka.tKortit as kortti, i(kortti.name)}
        ...
        <div class="flex flex-col items-center w-4xs h-6xs" in:fly|global={{ delay: i*100, duration: 50, x: 300, y: 0 }}>
            ...
            <button onclick={() => naytaKortti(kortti)}>
              <img class='cursor-pointer h-40 w-30 md:h-80 md:w-60 lg:w-75 lg:h-95 transition  duration-175 ease-in-out hover:scale-101 rounded-xl border-4 border-black outline-1 outline-[#FFD700] shadow-lg hover:shadow-[#FFD700]' src={kortti.image} alt="Kortin kuvateksti" /></button>
