...
<div class="p-10 mx-auto min-h-screen min-w-screen space-y-4 bg-radial from-[#472454] to-[#200f25] grid grid-cols-3 sm:grid-cols-6 gap-4 ">
    {#each pakka.tKortit as kortti, i(kortti.name)}
        <div class="flex flex-col items-center w-4xs h-6xs" in:fly|global={{ delay: i*100, duration: 50, x: 300, y: 0 }}>
            ...
            <button onclick={() => naytaKortti(kortti)}><img class='cursor-pointer h-40 w-30 md:h-90 md:w-60 lg:w-75 lg:h-100 transition  duration-175 ease-in-out hover:scale-101 rounded-xl border-4 border-black outline-1 outline-[#FFD700] shadow-lg hover:shadow-[#FFD700]' src={kortti.image} alt="Kortin kuvateksti" /></button>
